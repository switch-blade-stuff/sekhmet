...
namespace sek::math::detail
{
	...
	__m128 x86_exp_ps(__m128 v) noexcept
	{
		...
		poly = _mm_add_ps(_mm_add_ps(_mm_mul_ps(poly, c), a), one);
	...
	__m128 x86_log_ps(__m128 v) noexcept
	{
		...
		poly = _mm_add_ps(poly, _mm_mul_ps(c, _mm_set1_ps(logq1_f)));
		poly = _mm_sub_ps(poly, _mm_mul_ps(a2, _mm_set1_ps(0.5f)));
		a = _mm_add_ps(_mm_add_ps(a, poly), _mm_mul_ps(c, _mm_set1_ps(logq2_f)));
