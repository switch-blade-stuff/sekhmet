...
namespace sek::math::detail
{
	...
	inline namespace generic
	{
		...
		template<typename T, std::size_t N>
		constexpr void vector_rad(vector_data_t<T, N> &out, const vector_data_t<T, N> &l) noexcept
		{
			vector_unwrap<N>([&](auto i) { out[i] = rad(l[i]); });
		}
		template<typename T, std::size_t N>
		constexpr void vector_deg(vector_data_t<T, N> &out, const vector_data_t<T, N> &l) noexcept
		{
