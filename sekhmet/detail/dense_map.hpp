...
namespace sek
{
	...
	public:
		...
		constexpr explicit dense_map(const allocator_type &value_alloc, const bucket_allocator_type &bucket_alloc = {})
		 ...
		 * @param value_alloc Allocator used to allocate map's value array.
		 * @param bucket_alloc Allocator used to allocate map's bucket array. */
		constexpr dense_map(std::initializer_list<value_type> init_list,
							const key_equal &key_compare = {},
							const hash_type &key_hash = {},
							const allocator_type &value_alloc = {},
							const bucket_allocator_type &bucket_alloc = {})
			: dense_map(init_list.begin(), init_list.end(), key_compare, key_hash, value_alloc, bucket_alloc)
		{
		}
